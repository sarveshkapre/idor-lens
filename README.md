...
- For apps where status-only is too noisy (attacker gets a 2xx denial page), use `allow_contains` / `allow_regex` to only treat a role as allowed when 2xx + allow-match (and not deny-match).
